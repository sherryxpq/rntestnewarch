...
export function CallScreen({navigation, route}: CallScreenProps) {
  ...
  useEffect(() => {
    ...
    const userShareStatusChangeListener = zoom.addListener(
      ...
      async ({user, status}: {user: ZoomVideoSdkUser; status: ShareStatus}) => {
        ...
        if (user.userId && (status === ShareStatus.Start || status === ShareStatus.Resume)) {
  ...
  const onPressMore = async () => {
    ...
    let options = [
      ...
      {
        text: `${isReceiveSpokenLanguageContentEnabled ? 'Disable' : 'Enable'} receiving original caption`,
        onPress: async () => {
          await zoom.liveTranscriptionHelper.enableReceiveSpokenLanguageContent(!isReceiveSpokenLanguageContentEnabled);
          setIsReceiveSpokenLanguageContentEnabled(await zoom.liveTranscriptionHelper.isReceiveSpokenLanguageContentEnabled());
          console.log("isReceiveSpokenLanguageContentEnabled = " + isReceiveSpokenLanguageContentEnabled);
        },
      },
      // { text: 'Switch Camera', onPress: () => zoom.videoHelper.switchCamera() },
      {
        text: `${isOriginalAspectRatio ? 'Enable' : 'Disable'} original aspect ratio`,
        onPress: async () => {
          await zoom.videoHelper.enableOriginalAspectRatio(!isOriginalAspectRatio);
          setIsOriginalAspectRatio(await zoom.videoHelper.isOriginalAspectRatioEnabled());
      ...
      },
      {
        ...
        onPress: async () => {
          await zoom.chatHelper.changeChatPrivilege(ZoomVideoSDKChatPrivilegeType.ZoomVideoSDKChatPrivilege_Publicly);
        },
      }
    ];
  ...
  return (
    <View style={contentStyles}>
      ...
      <SafeAreaView style={styles.safeArea} pointerEvents="box-none">
        ...
        <Modal
          visible={vbModalVisible} transparent={true}>
